...
export default function SelectedSeiyuuVoices() {
  ...
  const searchedAnime = useMemo(() => {
    return selectedSeiyuu?.anime?.filter((anime) =>
      anime?.anime?.title.toLowerCase().includes(charQuery.toLowerCase()),
    );
  }, [charQuery, selectedSeiyuu]);
      ...
      {selectedSeiyuuLoading ? (
        ...
        <div className="mt-[48px]">
          ...
          <h1 className="text-[28px] text-left mb-[24px]">
            Anime related to <b>{selectedSeiyuu?.name}</b>
          </h1>
          ...
          <div className="grid gap-4 laptop_1300:grid-cols-2">
            {searchedAnime?.map((anime, i) => {
              return (
                <Link key={RandomID()} to={`/Anime/${anime?.anime?.mal_id}`}>
                  <div
